...
def Bar_Plot2(Data,Parameter):
    fig=px.bar(data_frame=Data,x='Day',y=Parameter,barmode='group',color='Time (in 24 hr format)', width=1100, height=400)
...
def Line_plot2(Data,Parameter):
    fig = px.line(data_frame=df, x='Day', y=Parameter, line_group='Time (in 24 hr format)',
                    color='Time (in 24 hr format)', width=1100, height=400)
    st.plotly_chart(fig)
